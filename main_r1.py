...
class CustomQNetwork(q_network.QNetwork):
    def __init__(self,
                 input_tensor_spec,
                 action_spec,
                 fc_layer_params=(2048, 2048, 2048),
                 dropout_rate=0.5,
                 l2_reg=0.01,
...
def main2():
    ...
    channel_mat = np.zeros((env_instance.transmitters, env_instance.users))
    ...
    for episode in range(num_simul_rounds):
        ...
        for iteration in range(num_iterations):
            ...
            if step % 100 == 0:
                print('episode = ', episode, 'step = {0}: state = {1}'.format(step, train_env_raw.state))
                print('episode = ', episode, 'step = {0}: channel gain = {1}'.format(step, train_env_raw.channel_gain))
                print('episode = ', episode, 'step = {0}: loss = {1}'.format(step, train_loss))
                print('episode = ', episode, 'step = {0}: DRL actions = {1}'.format(step, env_instance.decode_action(action_step.action.numpy())))
                #print('step = {0}: DRL actions = {1}'.format(step, env_instance.action_set[action_step.action.numpy()]))
                print('episode = ', episode, 'step = {0}: DRL reward = {1}'.format(step,rewards[episode, iteration]))
                print('episode = ', episode, 'step = {0}: Brute action = {1}'.format(step,best))
                print('episode = ', episode, 'step = {0}: Brute reward = {1}'.format(step, optimal_no_delay[episode, iteration]))
                print('episode = ', episode, 'step = {0}: Full reward = {1}'.format(step, full_pwr[episode, iteration]))
            '''
...
class CustomQNetworkmulti(TFModelV2):
    """Custom model for Q-network."""

    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        super(CustomQNetworkmulti, self).__init__(obs_space, action_space, num_outputs, model_config, name)

        # Use the Keras layers
        self.fc1 = tf.keras.layers.Dense(200, activation=tf.nn.relu)
        self.fc2 = tf.keras.layers.Dense(100, activation=tf.nn.relu)
        self.fc3 = tf.keras.layers.Dense(40, activation=tf.nn.relu)
        self.output_layer = tf.keras.layers.Dense(num_outputs, activation=None)

    def forward(self, input_dict, state, seq_lens):
        # Assume input_dict["obs"] is either a single observation or a batch of observations
        x = input_dict["obs"]
        if isinstance(x, dict):
            # If observations are in a dictionary format, concatenate them
            x = tf.concat(list(x.values()), axis=1)
...
ModelCatalog.register_custom_model("custom_q_network_multi", CustomQNetworkmulti)


def generate_lr_schedule(initial_lr, decay_rate, max_timestep, step_size):
    schedule = []
    for timestep in range(0, max_timestep, step_size):
        lr = initial_lr * (1 - decay_rate) ** (timestep / 3)
        schedule.append([timestep, lr])
    return schedule
...
def main_multi2():
    ...
    def create_env(env_config):
        # Ignore env_config since your environment doesn't use it
        return DRLmultienv()
    tune.register_env('DRLmutienv2', create_env)
    ...
    config = config.rollouts(num_rollout_workers=env_instance.transmitters)
    config = config.rollouts(rollout_fragment_length=1)
    config = config.environment(env="DRLmutienv2")
    ...
    config.replay_buffer_config.update(replay_config)

    config = config.framework(framework="tf")
    config = config.training(replay_buffer_config=replay_config)
    initial_learning_rate = 5e-3
    decay_rate = 1e-4
    max_timestep = env_instance.max_TTI * env_instance.transmitters  # Maximum timestep
    step_size = 1  # Step size for discretization
    lr_schedule = generate_lr_schedule(initial_learning_rate, decay_rate, max_timestep, step_size)
    config = config.training(lr_schedule=lr_schedule)
    config = config.training(gamma= 0.5)
    config = config.training(train_batch_size= 256)
    config = config.training(target_network_update_freq= 100 * env_instance.transmitters)
    config = config.training(model={"custom_model": "custom_q_network_multi",},)
    config = config.training(optimizer={"type": "adam",},)

    def policy_mapping_fn(agent_id, *args, **kwargs):
        # Define your policy mapping logic based on agent_id
        # For example, returning the same policy ID for all agents
        return "shared_policy"

    config = config.multi_agent(policies= {"shared_policy":(DQNTFPolicy, obs_space, act_space, config,)}, policy_mapping_fn=policy_mapping_fn)
    if config.exploration_config is None:
    ...
    config.exploration_config.update(explore_config)
    config = config.exploration(exploration_config=config.exploration_config)


    # Build the DQN trainer using the configured DQNConfig.
    dqn_trainer = DQNTrainer(config=config)
    ...
    for episode in range(num_simul_rounds):
        current_observation = train_env_raw.reset()
        ...
        for iteration in range(num_iterations):
            ...
            random_pwr[episode, iteration] = compute_sum_rate(train_env_raw.channel_gain, action_random,
                                                              env_instance.noise)
            #actions = dqn_trainer.compute_actions(current_observation)
            #current_observation, reward, done, info = train_env_raw.step(actions)

            result = dqn_trainer.train()

            #print(actions)
            #print(current_observation)
            print(train_env_raw.rates)
            ...
            if iteration % 100 == 0:
                ...
                print('episode = ', episode, 'iteration = {0}: channel gain = {1}'.format(iteration, train_env_raw.channel_gain))
                #print('episode = ', episode, f"Iteration: {iteration}, loss: {loss}")
                ...
                print('episode = ', episode, 'step = {0}: DRL reward = {1}'.format(iteration, rewards[episode, iteration]))
                print('episode = ', episode, 'step = {0}: Brute action = {1}'.format(iteration, best))
                print('episode = ', episode, 'step = {0}: Brute reward = {1}'.format(iteration, optimal_no_delay[episode, iteration]))
                print('episode = ', episode, 'step = {0}: Full reward = {1}'.format(iteration, full_pwr[episode, iteration]))
...
def plot_hexagonal_grid(tx_positions, rx_positions, inside_status, R, r):
    ...
    tx_colors = ['lightblue', 'blue', 'darkblue']
    rx_colors = ['lightcoral', 'red', 'darkred']

    dot_size = 50
    # Plot Tx positions and draw hexagons and circles
    for i, tx in enumerate(tx_positions):
        tx_label = f'Tx{i + 1}'
        ax.scatter(*tx, color=tx_colors[i % len(tx_colors)], label=tx_label, s=dot_size)
    ...
    # Plot Rx positions
    for i, (rx, inside) in enumerate(zip(rx_positions, inside_status)):
        rx_label = f'Rx{i + 1}'
        color = rx_colors[i % len(rx_colors)] if inside else 'orange'  # Orange for Rx outside the hexagon
        ax.scatter(*rx, color=color, label=rx_label, s=dot_size)

    ax.set_aspect('equal', 'box')
    legend_font_size = 20  # You can adjust this value
...
def testing():
    ...
    if env.observation_space.contains(observation):
        print("The observation is within the defined Gymnasium Space.")
    else:
        print("The observation is not within the defined Gymnasium Space.")

    for step in range(num_steps):
        # Create a random action for each agent
        actions = {agent_id: env.action_space.sample() for agent_id in observation.keys()}
