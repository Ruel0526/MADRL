...
from DRL_multi_learn_r3 import DRLmultiagent
...
def main_multi():
    ...
    dqn_multi = DRLmultiagent(transmitters * 2, 10, action_cand, pmax, noise)
    ...
    for i in range(num_simul_rounds):
        Return = 0
        states_of_agents = np.zeros((transmitters, users * 2))#.flatten()
        ...
        for j in range(num_TTIs):
            ...
            for x in range(users):
                action_of_agent = actions_of_agents[x]
                inter = 0
                ...
                for y in range(transmitters):
                    if y == x:
                        ...
                        action_of_interferer = actions_of_agents[y]
            ...
            sum_rate_of_DRL[i, j] = sum_rate_temp
            ...
            state_transit = np.zeros((transmitters, users * 2))
            final_reward = 0
            for k in range(transmitters):
                #print('iteration =', j, 'agent=', k, 'current state =', states_of_agents[k, :])
                #print('iteration =', j, 'agent=', k, 'new state=', next_state)

                next_state, reward, done, info = dqn_multi.step(states_of_agents, actions_of_agents, j, num_TTIs,
                                                                channel_gain, k)
                #print('iteration =', j, 'agent=', k, 'current state =', states_of_agents[k, :])
                #print('iteration =', j, 'agent=', k, 'new state=', next_state)
            ...
            for k in range(1, action_cand):
                for l in range(1, action_cand):
                    for m in range(1, action_cand):
                        x = action_set[m]
                        y = action_set[l]
                        z = action_set[k]
            ...
            sum_rate_temp = 0
            for x in range(users):
                action_of_agent = action_set[random.randint(1, action_cand - 1)]
                inter = 0
                direct_signal = channel_gain[x, x] * action_of_agent
                for y in range(transmitters):
                    if y == x:
                        ...
                        action_of_interferer = action_set[random.randint(1, action_cand - 1)]
                        gain_temp_interferer = channel_gain[y, x]
                        inter_of_interferer = gain_temp_interferer * action_of_interferer
                        inter += inter_of_interferer
...
def graph(switch):
    ...
    if switch == 0:
        #plt.plot(range(start, len(reward_avg_multi)), cumulative_rewards_multi, label='Multi-agent DRL')
        plt.plot(range(start, len(reward_avg_multi_rate)), cumulative_rate_multi, label='Multi-agent DRL')
        plt.plot(range(start, len(reward_avg_optimal_no_delay)), cumulative_rewards_optimal_no_delay, label='Brute (no delay)')
    ...
    if switch == 1:
        ...
        plt.plot(range(space - 1, space - 1 + len(cumulative_rate_multi)), cumulative_rate_multi, label='Multi-agent DRL')
        plt.plot(range(space - 1, space - 1 + len(cumulative_rewards_optimal)), cumulative_rewards_optimal, label='Brute (delay)')
        plt.plot(range(space - 1, space - 1 + len(cumulative_rewards_optimal_no_delay)), cumulative_rewards_optimal_no_delay, label='Brute (no delay)')
        plt.plot(range(space - 1, space - 1 + len(cumulative_rewards_full_pwr)), cumulative_rewards_full_pwr, label='Full power')
        plt.plot(range(space - 1, space - 1 + len(cumulative_rewards_random_pwr)), cumulative_rewards_random_pwr, label='Random power')
